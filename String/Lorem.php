...
class Lorem
{
    public static function ipsum($nparagraphs, $nsentences = null, $nwords = null)
    {
        $paragraphs = [];
        for ($p = 0; $p < $nparagraphs; ++$p) {
            $nsentences = is_null($nsentences) ? random_int(3, 8) : $nsentences;
            $sentences = [];
            for ($s = 0; $s < $nsentences; ++$s) {
                $frags = [];
                /*$commaChance = .33;*/
                while (true) {
                    $nwords = is_null($nwords) ? random_int(3, 15) : $nwords;
